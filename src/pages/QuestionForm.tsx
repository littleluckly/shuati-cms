...
import { 
  ...
  Radio, Space, message, Divider, InputNumber, Checkbox
} from 'antd';
...
const mockQuestions = [
  {
    ...
    answer: "useState是React中的一个钩子函数，用于在函数组件中添加状态管理。它返回一个包含当前状态和更新状态的函数的数组。",
    explanation: "useState是React Hooks的一部分，允许函数组件拥有自己的状态，而无需将其转换为类组件。",
    status: "已发布"
  }
];
...
const QuestionForm = () => {
    ...
    form.validateFields()
      .then(values => {
        ...
        if (questionType === '选择题' || questionType === '多选题') {
          ...
          if (questionType === '选择题' && options.filter(option => option.isCorrect).length > 1) {
        ...
        const questionData = {
          ...
          answer,
          explanation,
          options: questionType === '选择题' || questionType === '多选题' ? options : [],
  ...
  const handleTypeChange = (type) => {
    if (type === '选择题') {
      // 确保至少有2个选项，且只有一个正确
      const newOptions = options.length >= 2 ? [...options] : [
        ...options,
        { id: uuidv4(), text: '', isCorrect: false }
      ];
    ...
    <div>
      ...
      <Card>
        ...
        >
          ...
          {/* 选项区域 - 仅选择题和多选题显示 */}
          {form.getFieldValue('type') === '选择题' || form.getFieldValue('type') === '多选题' ? (
            <Form.Item label={`${form.getFieldValue('type') === '选择题' ? '单选题' : '多选题'}选项`}>
              <div>
                {options.map((option, index) => (
                  <div key={option.id} style={{ display: 'flex', alignItems: 'center', marginBottom: 12 }}>
                    {form.getFieldValue('type') === '选择题' ? (
                      <Radio 
                        checked={option.isCorrect}
                        onChange={(e) => updateOptionCorrectness(option.id, e.target.checked)}
                        style={{ marginRight: 8 }}
                      >
